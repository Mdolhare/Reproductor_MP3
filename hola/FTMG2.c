...
void FTM_ConfigEPWM(FTMn_t ftm_id, uint16_t mod, FTMChannel_t chn){
	...
	ftm->PWMLOAD = FTM_PWMLOAD_LDOK_MASK | 0x0F;//FTM_PWMLOAD_CHnSEL(chn);
	...
	//
	//Config sync PWM
	ftm->SYNC |= FTM_SYNC_CNTMAX_MASK | FTM_SYNC_TRIG2_MASK | FTM_SYNC_TRIG1_MASK;
	...
	FTM_SetInterruptMode(ftm, chn, true);
	FTM_DmaMode(ftm, chn, true);
