 ...
 *  Author: https://github.com/ChuOkupai/n-ary-tree/blob/master/node.c
 *  Modificado por Grupo 2
 */
...
Node*	nodeNew(char *name, char *path)
{
	Node *n;
...
Node*	nodeFind(Node *node, char *name, char *path, int (*compare)(void *a, void *b))
{
	if (! node)
		return node;
	if (! compare(name, node->name))
		return node;
	if (! compare(path, node->path))
			return node;
	if (node->next)
		return nodeFind(node->next, name, path, compare);
	if (node->children)
		return nodeFind(node->children, name, path, compare);
	return NULL;
}
